Toadcoin Core version *0.15.1* is now available from:

  <https://download.toadcoin.org/toadcoin-0.15.1/>
...
A number of changes to the way Toadcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 hops per kB (0.00001 TOAD). This is to prepare for dropping the minimum transaction fee to 0.1 hops per kB (0.0001 TOAD) in the next release.
